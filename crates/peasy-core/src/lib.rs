    ...
    pub fn parse(value: &str) -> Result<Self, ValidationError> {
        match value {
            ...
            _ => Err(ValidationError::InvalidRequest(format!(
                "unsupported GNOME accent colour `{value}`"
            ))),
        }
    }
}
...
impl ColorScheme {
    pub fn parse(value: &str) -> Result<Self, ValidationError> {
        match value {
            ...
            "dark" => Ok(Self::Dark),
            _ => Err(ValidationError::InvalidRequest(format!(
                "unsupported GNOME colour scheme `{value}`"
            ))),
        }
    }
...
fn nix_string(value: &str) -> String {
    serde_json::to_string(value).expect("serializing validated Nix string")
}
...
pub fn render_system_expression(
    ...
    let nixpkgs = serde_json::to_string(&nixpkgs_path.to_string_lossy()).expect("path JSON");
    let host = serde_json::to_string(&host_configuration.to_string_lossy()).expect("path JSON");
